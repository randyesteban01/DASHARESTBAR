object frmEntradas: TfrmEntradas
  Left = 489
  Top = 306
  Width = 1228
  Height = 480
  Caption = 'Entradas'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  KeyPreview = True
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1212
    Height = 90
    Align = alTop
    TabOrder = 0
    object btclose: TSpeedButton
      Left = 8
      Top = 8
      Width = 73
      Height = 73
      Caption = 'F1-SALIR'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      ParentFont = False
      OnClick = btcloseClick
    end
    object btedit: TSpeedButton
      Left = 88
      Top = 8
      Width = 73
      Height = 73
      Caption = 'F2-CAMBIAR'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      ParentFont = False
      OnClick = bteditClick
    end
    object btadd: TSpeedButton
      Left = 168
      Top = 8
      Width = 73
      Height = 73
      Caption = 'F3-NUEVO'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      ParentFont = False
      OnClick = btaddClick
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 90
    Width = 1212
    Height = 351
    Align = alClient
    DataSource = dsEntradas
    Options = [dgTitles, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    ReadOnly = True
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'EntradaID'
        Title.Caption = 'ID'
        Width = 50
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Fecha'
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NombreProveedor'
        Title.Caption = 'Nombre del Proveedor'
        Width = 150
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Concepto'
        Width = 200
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Exento'
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Grabado'
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Itbis'
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Descuento'
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Total'
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'RealizadoPor'
        Width = 200
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'RevisadoPor'
        Width = 200
        Visible = True
      end>
  end
  object QEntradas: TADOQuery
    Connection = DM.ADOBar
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT Entradas.EntradaID, Entradas.Fecha, Entradas.Concepto,'
      
        ' Entradas.RealizadoID, ura.Nombre+'#39' : '#39'+CAST(Entradas.RealizadoI' +
        'D as varchar) AS RealizadoPor,'
      
        ' Entradas.RevisadoID,  ure.Nombre+'#39' : '#39'+CAST(Entradas.RevisadoID' +
        ' as varchar) AS RevisadoPor,'
      
        ' Entradas.ProveedorID, Proveedores.Nombre+'#39' : '#39'+CAST(Entradas.Pr' +
        'oveedorID as varchar) AS NombreProveedor,'
      
        ' Entradas.Exento, Entradas.Grabado, Entradas.Itbis, Entradas.Des' +
        'cuento, Entradas.Total, Entradas.ExistenciaAplicada'
      'FROM Entradas '
      'LEFT JOIN Usuarios ura ON Entradas.RealizadoID = ura.UsuarioID'
      'LEFT JOIN Usuarios ure ON Entradas.RevisadoID  = ure.UsuarioID '
      
        'LEFT JOIN Proveedores  ON Entradas.ProveedorID = Proveedores.Pro' +
        'veedorID'
      'order by ExistenciaAplicada desc')
    Left = 240
    Top = 152
    object QEntradasEntradaID: TAutoIncField
      FieldName = 'EntradaID'
      ReadOnly = True
    end
    object QEntradasFecha: TDateTimeField
      FieldName = 'Fecha'
    end
    object QEntradasProveedorID: TIntegerField
      FieldName = 'ProveedorID'
    end
    object QEntradasNombreProveedor: TWideStringField
      FieldName = 'NombreProveedor'
      Size = 255
    end
    object QEntradasConcepto: TWideStringField
      FieldName = 'Concepto'
      Size = 255
    end
    object QEntradasExento: TBCDField
      FieldName = 'Exento'
      Precision = 19
    end
    object QEntradasGrabado: TBCDField
      FieldName = 'Grabado'
      Precision = 19
    end
    object QEntradasItbis: TBCDField
      FieldName = 'Itbis'
      Precision = 19
    end
    object QEntradasDescuento: TBCDField
      FieldName = 'Descuento'
      Precision = 19
    end
    object QEntradasTotal: TBCDField
      FieldName = 'Total'
      Precision = 19
    end
    object QEntradasRealizadoID: TIntegerField
      FieldName = 'RealizadoID'
    end
    object QEntradasRealizadoPor: TWideStringField
      FieldName = 'RealizadoPor'
      Size = 255
    end
    object QEntradasRevisadoID: TIntegerField
      FieldName = 'RevisadoID'
    end
    object QEntradasRevisadoPor: TWideStringField
      FieldName = 'RevisadoPor'
      Size = 255
    end
    object QEntradasExistenciaAplicada: TBooleanField
      FieldName = 'ExistenciaAplicada'
    end
  end
  object dsEntradas: TDataSource
    DataSet = QEntradas
    OnDataChange = dsEntradasDataChange
    Left = 272
    Top = 152
  end
end
